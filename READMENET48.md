...
I specifically said "publish" here, instead of "develop", since the source code is 99% the same, for both. This is often the case, for rather simple CLI tools, which not make any use of modern C# features, which normally offer great QoL improvements. Otherwise the following thoughts would not come into my mind and would not stand up to below argumentation.
