    ...
    public sealed class Main
    {
        public static void Run(string[] args)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine();
                    Console.WriteLine($"Display description: {monitor.GetMonitorDescription()}");
                    Console.WriteLine($"Current VCP60 value: {monitor.GetVCP60Value()}");
                }
                else if (isSet)
                {
                    ...
                    monitor.SetVCP60Value(value);
                }
                else if (isCap)
                {
                    var capString = monitor.GetCapabilitiesString();
